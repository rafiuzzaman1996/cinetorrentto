...
import GenreColumns from './column'
import { AddEditDialog } from './AddEditDialog'
import { DataTableSkeleton } from '@/components/admin/management/data-table/data-table-skeleton'
import { FilterSearch } from './FilterSearch'
import { getGenres } from '../../admin-api/GenreApi'

const AdminGenreListPage = async (props: { searchParams: Promise<{ page?: string; pageSize?: string, search?: string }> }) => {
  const searchParams = await props.searchParams
  const pageIndex = Number(searchParams?.page ?? "1") // page number from URL
  const pageSize = Number(searchParams?.pageSize ?? "20")

  const getCategoriesData = await getGenres({
    page: pageIndex,
    limit: pageSize,
