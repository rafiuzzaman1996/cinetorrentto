...
import { getFeaturedContents } from '../../admin-api/FeaturedContentApi'
import FeaturedContentColumns from './column'

const AdminFeaturedContentListPage = async (props: { searchParams: Promise<{ page?: string; pageSize?: string, search?: string }> }) => {
  const searchParams = await props.searchParams
  const pageIndex = Number(searchParams?.page ?? "1") // page number from URL
  const pageSize = Number(searchParams?.pageSize ?? "20")

  const getCategoriesData = await getFeaturedContents({
    page: pageIndex,
    limit: pageSize,
  ...
  return (
    <div className="p-4 pt-0">
      ...
      >
        <SimpleDataTable
          caption='A list of Featured Content.'
          columns={FeaturedContentColumns}
          // schema={schema}
...
export default AdminFeaturedContentListPage