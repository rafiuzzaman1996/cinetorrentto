...
export function SocialLinkColumns() {
  const router = useRouter();

  async function handleSubmitForm(values: SocialLink, mode: "add" | "edit" | "view" | "delete") {
    try {
      await submitSocialLink(values, mode); // API call for all modes
      toast.success(
...
export const SocialLinkColumns2: ColumnDef<z.infer<typeof schema>>[] = [
  // {
  //   accessorKey: "id",
  //   header: "ID",
  //   enableHiding: true,
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-2">
          <AddEditDialog mode="view" data={link} />
          <AddEditDialog mode="edit" data={link} />
          <AddEditDialog mode="delete" data={link} onSubmit={} />
        </div>
      )
    }
  }
